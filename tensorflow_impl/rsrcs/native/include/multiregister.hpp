...
**/
EXCEPTION(MultiRegister, Any, "multi-replica exception");
    EXCEPTION(MultiRegisterClosed, MultiRegister, "multi-replica was/has been destroyed");
    EXCEPTION(MultiRegisterTimeout, MultiRegister, "wait for multi-replica timed out");
    EXCEPTION(MultiRegisterUnreachable, MultiRegister, "unreachable code reached (i.e. bug)");
 ...
 * @param Type Element class, must be trivially destructible
**/
...
public:
    ...
    **/
    template<class... Args> Array(size_t length, Args&&... args): storage{new Store[length]}, length{length} {
        for (size_t i = 0; i < length; ++i)
            new(&storage[i]) Type(::std::forward<Args>(args)...);
    }
    /** Element destructor.
    **/
...
protected:
    ...
    **/
    ...
    /** Wait for a condition variable, check if still open.
